...
# Compatibility shim — hidapi and hid packages have slightly different APIs.
# We wrap them here so the rest of the code works identically on both.
class _HidCompat:
    """Wraps both 'hid' (Linux/macOS) and 'hidapi' (Windows) packages."""

    def enumerate(self, vid, pid):
        devs = _hid_module.enumerate(vid, pid)
        result = []
        for d in devs:
            if isinstance(d, dict):
                result.append(d)
            else:
                # hidapi package returns objects, not dicts
                result.append({
                    'path':             getattr(d, 'path', b''),
                    'usage_page':       getattr(d, 'usage_page', 0),
                    'interface_number': getattr(d, 'interface_number', -1),
                })
        return result

    def open(self, path):
        # hid package: hid.device().open_path()
        # hidapi package: hid.Device(path=path)
        if hasattr(_hid_module, 'device'):
            dev = _hid_module.device()
            dev.open_path(path)
            return dev
        else:
            return _hid_module.Device(path=path)
...
class BeastXDevice:
    ...
    def find(self):
        """Find the mouse — returns info dict or None."""
        devs = hid.enumerate(VID, PID)
        if not devs:
            return None
        for d in devs:
            if d.get('usage_page', 0) == 0xFF00 or d.get('interface_number', -1) == 1:
                return d
        return devs[0]
    ...
    def connect(self):
        ...
        self._dev = hid.open(info['path'])
        # set_nonblocking only exists on hid package
